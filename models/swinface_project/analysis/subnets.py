...
class TaskSpecificSubnet(torch.nn.Module):
    ...
    def forward(self, x):
        #print("Inside TaskSpecificSubnet",x.size(),"///",x) #8*512
...
class TaskSpecificSubnets(torch.nn.Module):
    ...
    def forward(self, x):
        #print("Inside TaskSpecificSubnets",x.size(),"///",x) #11*8*512
        outputs = []
        for i in range(self.branch_num):
            net = self.nets[i]
...
class OutputModule(torch.nn.Module):
    ...
    def old_forward(self, x, embedding):
        #print("Inside OutputModule",x.size(),"///",x) #11*8*512
...
class ModelBox(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        attributes_11_512,id_embeddings = self.om(x, embedding)
        return attributes_11_512,id_embeddings